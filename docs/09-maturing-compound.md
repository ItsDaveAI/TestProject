...
**The recipe format, exactly (client data):** `Compound_Rare` (187 recipes) and `Compound_Potion` (155) define the non-Nate compound economy — each recipe carries up to **3 result items at a result level**, up to **5 request items with counts**, a **Probability (percent)**, a **Fee in galders**, and a **WasteItem** — the byproduct you're left with on failure. One verbatim example: **힘을 주는 반지 ("Strength-granting Ring")** = Addax's Horn ×4 + Steel Fragment ×4 + Crystal ×3 + Gold Ring ×2 + Ampoule ×5, at **30% success, 700 galders fee**. `Compound_Throw` (19) covers the throwing-weapon recipes.
  ...
  The compound window **displays the result percentage** (`CompoundWindow`'s ResultPercent field) — the client showed you the odds before you rolled.
...
**Ghost growth-stones (client `GhostExchange_Group`, 36 groups):** a late-era exchange layer organizes the compound **growth-stone (성장조합석) equipment by level bracket** — e.g. group 20 binds the **Lv 290 stone family** (12 item IDs) for ghost-equipment exchange, gated by refine level (`GhostExchange_RefineLevel`) — the Ghost-equipment upgrade economy's grouping table. The refine side (`GhostUIParams`) offers **directed refining**: choose 없음 ("none") / 공격력 ("attack power") / 방어력 ("defense") — a branch distinct from standard refinement's uniform upgrades, with its own exchange error set (wrong count, non-exchangeable item, inventory overflow).
