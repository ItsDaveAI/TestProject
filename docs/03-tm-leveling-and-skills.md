...
**Warp-gate skills (client `WarpGateSkillInfo`, 52 rows):** jobs carry **class-specific warp skills** — per row: the warp type, **JobType**, and destination zone (the job-teleport family binding each of the 36 job types to its zones). **Imprint skills (client `ImprintSkillInfo`, 35):** a timed-buff family — e.g. *공격의 각인* ("Imprint of Attack") with a **24-hour duration** — the Tartarus-era imprint buffs.

**The per-level skill curves (client `ESAll_` / `ESAII_` / `ESAction_` families — 836 files):** every skill's level-by-level numbers exist in data, one file per skill: **MP cost, cast delay, range, cooldown, and monster-delay per level** (plus effect ratios in `ESAction_`, e.g. Enchanted CDM's 30% → 33% → 36%… ladder). A verbatim example — **Uppercut** (`ESAll_UpperCut`): **Lv 1 = 140 MP, 23 s cooldown; Lv 10 = 230 MP, 14 s cooldown** — matching the Korean guide's "even mastered, the cooldown stays ~11–14 s." These 836 files are the numeric floor under every skill formula in this file; `ESAII_Mon_*` carries the same curves for monster skills.
