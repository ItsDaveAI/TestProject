...
**The Tartarus floor infrastructure (client data):** the dungeon's per-floor roster is fully tabulated — **97 `Mon_Tartaros1/2_*` monster tables** and **97 `BossMon_Tartaros1/2_*` boss-spawn tables (183 spawn rows — Tartarus 1's 55 floors carry 132 of them)**, with **63 `NpcMsg_Tartaros*` + 49 `NpcTalk_Tartaros*` dialog sets** for the floor NPCs (the Npc425 crack dialogs above are one voice in that set) and the **Tartarus compounder shop** (`Compounder_Shop_Tartaros`) for the in-dungeon crafting counter.
...
**Earlier town bathhouses (client data):** Tapasco's Theme Spa wasn't the island's first bathhouse — the client preserves two town spas of its own: **Paradise's 목욕탕** (`NpcMsg_Beach_Town_Spa_Npc26` — its self-introduced "CEO" runs a buried-letter quest, paying you to dig up *other people's letters* from Desert Beach Field 4's Parasol Beach) and the **Relics Town 찜질방** (`NpcMsg_Relics_Town_Spa_Npc27` — a shy attendant who handed over Hwade candy without managing the confession, vowing "next year for sure!"). Both carry full dialog sets and their own map-sound tables (`MapSound_*_Town_Spa`).
