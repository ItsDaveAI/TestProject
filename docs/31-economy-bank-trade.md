...
**The personal shop, fully documented (client `PlayerShopStrings`/`PlayerShopUIParams`/`PlayerShopWindow`):** the "thinly documented" era mechanics above are the client's own shop UI:

- **Opening and running:** the shop **must be named** ("상점의 이름을 지어주세요") and stocked before registration (`상점등록`); it runs in **owner mode (주인모드)** showing **income (수입금액), remaining weight and money, settlement weight/price, and deducted weight** — the shop settles transactions live while you stand at the stall. A shop has a **usable-time countdown** ("상점 이용 가능시간: %d분 %d초") — shops expired — and closes via a confirmed **shop-closure** action. You **cannot open a shop if selling everything would exceed your max carried galders** (`NotOpen_MaxGelder`), and the UI tracks mode changes with confirmation dialogs.
- **The built-in shop finder:** the in-client **개인 상점 검색** window searches every open shop by **15 item categories** (weapons, guns, hats, shields, capes, accessories, armor, pets, head, face, drills, fashion, potions, cards, etc.), **star grade (1–5성급 이상)**, **level range**, seller name, price, and stock — with a **buy button inside the search results** and sortable columns (view/name/price/stock/buy-count). The `/상점검색` chat command above is its quick path (search terms need **2+ characters** per `FindShopHelperStr`).
...
- Client data: `PlayerShopStrings` / `PlayerShopUIParams` / `PlayerShopWindow` (personal-shop flow, owner-mode income, usable-time countdown, in-client shop search), `FindShopHelperStr`, `PurchaseShopUIParams` (MyShop sets/gift memos), `PurShopUIDefine` (category tree), `RShopUIParams` / `RShopString` (알샵)
